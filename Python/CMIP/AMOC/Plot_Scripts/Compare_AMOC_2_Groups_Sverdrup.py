...
Fig. 1 in Eiselt and Graversen (2023), JCLI.

Be sure to adjust data_dir, pl_path, and possibly data_path.
"""
...
for g_n, group in zip(["G1", "G2"], [models1, models2]):
    ...
    for i, mod_d in enumerate(group):
        ...
        amoc_pic_run[:, i] = damoc_nc.variables["amoc_pic_run"][:] / h2o_rho /  1e6
        damoc[:, i] = damoc_nc.variables["damoc"][:] / h2o_rho /  1e6
...
#%% print the mean AMOC change over years 13-17
ye = 12
yl = 17
g1_e_damoc = np.mean(avg_d_g1[ye:yl])
g2_e_damoc = np.mean(avg_d_g2[ye:yl])
print(f"G1 early (y {ye+1}-{yl}) AMOC change: {g1_e_damoc}")
print(f"G2 early (y {ye+1}-{yl}) AMOC change: {g2_e_damoc}")
...
sle1, yie1, rve1, pve1, erre1 = lr(np.arange(ep), avg_a4x_g1[:ep]) 
sll1, yil1, rvl1, pvl1, errl1 = lr(np.arange(150-lp), avg_a4x_g1[lp:]) 
sle2, yie2, rve2, pve2, erre2 = lr(np.arange(ep), avg_a4x_g2[:ep]) 
sll2, yil2, rvl2, pvl2, errl2 = lr(np.arange(150-lp), avg_a4x_g2[lp:]) 
...
for g12 in ["G1", "G2"]:
    ...
    for mem in np.arange(len(models[g12])):
        ...
        sle[g12][mem], yie, rve, pve, erre = lr(np.arange(ep), amoc_a4x_dic[g12][:ep, mem]) 
        sll[g12][mem], yil, rvl, pvl, errl = lr(np.arange(150-lp), amoc_a4x_dic[g12][lp:, mem]) 
...
fig.suptitle("AMOC G1$-$G2 comparison", fontsize=17)
...
axes[0, 0].set_ylabel("AMOC in Sv", fontsize=14)
axes[0, 1].set_ylabel("AMOC in Sv", fontsize=14)
axes[1, 0].set_ylabel("AMOC change in Sv", fontsize=14)
axes[1, 1].set_ylabel("AMOC (change) difference in Sv", fontsize=14)
...
pl.savefig(pl_path + "AMOC_G1_G2_Comparison_V2.pdf", bbox_inches="tight", dpi=250)
