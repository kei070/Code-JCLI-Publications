"""
Calculate the water vapour radiative response for CMIP members via the kernel method.
...
out_path = f"/Kernel_{toa_sfc}_RadResp/{exp_n}/{k_p[kl]}/Q_Response/{cs_pa}{strat_path}/{out_path_ad[kl][k_st]}/"
...
if len(dqdt_f_list) > 1:
    for i in np.arange(1, len(dqdt_f_list)):
        dqdt = da.concatenate([dqdt, da.ma.masked_array(Dataset(dqdt_f_list[i]).variables[dq_str + "dT"], lock=True)])
...
p1 = ax1.contourf(x, y, np.nanmean(np.nanmean(dq[-12:, :, :, :].compute(), axis=0), axis=-1), levels=levels, norm=norm, 
                  cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
cb1.set_label("dlogQ in log(kg/kg)")
ax1.set_title(nl.models[mod] + " dlogQ Last Year Mean Zonal Mean " + nl.models_pl[mod])
pl.show()
pl.close()
# raise Exception
...
ax1.set_title(nl.models[mod] + " dlogQ/dT Last Year Mean Zonal Mean " + nl.models_pl[mod])
pl.show()
pl.close()
...
#%% multiply the kernel on the dQ change
#   --> NEW METHODOLOGY: multiply the separately calculated dQ/dT here
q_lw_toa = qke_lw_rep / dqdt * dq
q_sw_toa = qke_sw_rep / dqdt * dq
...
p_ind = 69 * 12 + 3
l_num = 14
levels, norm, cmp = set_levs_norm_colmap(sig[kl]*q_lw_toa_tot[p_ind, :, :].compute(), l_num, c_min=None, c_max=None, 
                                         quant=0.99)
...
p1 = ax1.contourf(x, y, sig[kl]*q_lw_toa_tot[p_ind, :, :].compute(), levels=levels, norm=norm, cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
ax1.coastlines(resolution="50m", color="black", linewidth=0.5, zorder=101)
cb1.set_label("Q LW response in Wm$^{-2}$K$^{-1}$")
ax1.set_title(nl.models[mod] + f" {cs_p} Q LW  {toa_sfc} Reponse First Time Step {kl} Kernels\nGlobal Mean: " + 
              str(glob_mean(sig[kl]*q_lw_toa_tot[p_ind, :, :].compute(), lat, lon)) + " Wm$^{-2}$")
pl.show()
pl.close()

print(glob_mean(sig[kl]*q_lw_toa_tot[p_ind, :, :].compute(), lat, lon))
# raise Exception("")


#%% a final test plot
l_num = 14
levels, norm, cmp = set_levs_norm_colmap(sig[kl]*q_sw_toa_tot[6, :, :].compute(), l_num, c_min=None, c_max=None, 
                                         quant=0.99)
...
p1 = ax1.contourf(x, y, sig[kl]*q_sw_toa_tot[6, :, :].compute(), levels=levels, norm=norm, cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
ax1.coastlines(resolution="50m", color="black", linewidth=0.5, zorder=101)
cb1.set_label("Q SW response in Wm$^{-2}$K$^{-1}$")
ax1.set_title(nl.models[mod] + f" {cs_p} Q SW {toa_sfc} Reponse First Time Step {kl} Kernels\nGlobal Mean: " + 
              str(glob_mean(sig[kl]*q_sw_toa_tot[6, :, :].compute(), lat, lon)) + " Wm$^{-2}$")
pl.show()
pl.close()

print(glob_mean(sig[kl]*q_sw_toa_tot[6, :, :].compute(), lat, lon))
# raise Exception

#%% store the water vapour radiative response in a netcdf file

print("\nGenerating nc file...\n")
out_name = toa_sfc + "_RadResponse_q" + exp_s + "_" + kl + out_ad[kl] + "_" + cs_n + nl.models[mod] + ".nc"
...
q_lw_resp_nc = f.createVariable("q_lw_resp", "f4", ("time", "lat", "lon"))
q_sw_resp_nc = f.createVariable("q_sw_resp", "f4", ("time", "lat", "lon"))
...
for i in np.arange(0, n_yr*12, n):
    ...
    # print("Months " + str(i+1) + " to " + str(i + n) + f" of {str(n_yr*12)} stored")
...
f.description = (f"This file contains the {toa_sfc} long and short wave {sky_desc} radiative response due to Q change "
                 f"\nin the {exp_n} experiment for " + nl.models_pl[mod] + ". " + dq_str + "/dT was used." +
