...
Be sure to set adj_path, cre_path, kern_path, and out_path.
"""
...
# adjustment path
# -->  path to clear-sky minus all-sky radiative responses generated with
# Calc_SurfaceAlbedo_Resp_KernelMethod_CSDiff.py
# Calc_Temp_Resp_KernelMethod_CSDiff.py
# Calc_WaterVapour_Resp_KernelMethod_CSDiff.py
adj_path = f"/Kernel_{toa_sfc}_RadResp/{exp_n}/{k_p[kl]}/01_Diff_to_CS/{nl.models[mod]}/{out_path_ad[kl][k_st]}/"

# CRE path
# --> path to CRE generated with Calc_and_Store_CRE.py
cre_path = f"/CRE{sfc_add}/{exp_n}/"

# kernel path
kern_path = f"/{k_p[kl]}_Kernels/{k_ad[kl]}"

# output path
out_path = f"/Kernel_{toa_sfc}_RadResp/{exp_n}/{k_p[kl]}/Cloud_Response/{out_path_ad[kl][k_st]}/"


#%% load nc files

# kernel calculate quantities
t_adj_nc = Dataset(adj_path + f"Diff_to_CS{sfc_add}_ta_ts{exp_s}_{kl}{fn_ad[kl]}_{nl.models[mod]}.nc")
q_adj_nc = Dataset(adj_path + f"Diff_to_CS{sfc_add}_q{exp_s}_{kl}{fn_ad[kl]}_{nl.models[mod]}.nc")
a_adj_nc = Dataset(adj_path + f"Diff_to_CS{sfc_add}_sa{exp_s}_{kl}{fn_ad[kl]}_{nl.models[mod]}.nc")

# cloud radiative effect
cre_nc = Dataset(cre_path + f"CRE{sfc_add}_Change_piC_{exp}_{nl.models[mod]}.nc")
...
pl.plot(c_re_gl_an)
pl.plot([0, n_yr], [0, 0], linewidth=0.75, c="gray", linestyle="--")
...
print("\nGenerating nc file...\n")
out_name = f"{toa_sfc}_RadResponse_Cloud" + exp_s + "_" + kl + out_ad[kl] + "_" + nl.models[mod] + ".nc"
...
c_re_nc = f.createVariable("c_resp", "f4", ("time", "lat", "lon"))
c_sw_re_nc = f.createVariable("c_sw_resp", "f4", ("time", "lat", "lon"))
c_lw_re_nc = f.createVariable("c_lw_resp", "f4", ("time", "lat", "lon"))
co2_c_masking_nc = f.createVariable("co2_masking", "f4", ("time2", "lat", "lon"))

# pass the data into the variables
lat_nc[:] = lat
lon_nc[:] = lon
tim_nc[:] = np.arange(n_yr*12)
c_re_nc[:] = ne.evaluate("cre_sw + a_re + q_sw_re") + ne.evaluate("cre_lw + t_re + q_lw_re")
...
tim_nc.units = t_adj_nc.variables["time"].units
...
f.description = (f"This file contains the adjusted cloud radiative {toa_sfc} response calculated following " + 
                 f"Soden et al. (2008) in the {exp_n} experiment for " + nl.models_pl[mod] + co2_mask_desc +
                 "The values correspond to monthly means.\nThe kernels are taken from Soden et al. (2008).")
