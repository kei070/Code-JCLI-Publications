"""
Calculate the surface albedo radiative "cloud adjustment" for CMIP members via the kernel method.
...
#%% set directory for kernels
direc = "/media/kei070/Seagate/"
...
# output path
out_path = f"/Kernel_{toa_sfc}_RadResp/{exp_n}/{k_p[kl]}/01_Diff_to_CS/{nl.models[mod]}/{out_path_ad[kl][k_st]}/"
...
# model latitudes and longitudes
...
# alb_pi = np.array(rsus_pi / rsds_pi)
...
toa_ch = np.zeros(np.shape(alb_ch))

count = 0
for i in np.arange(0, int(n_yr*12), 12):
    
    toa_ch[i:i+12, :, :] = alb_ch[i:i+12, :, :] * rem_alke
...
toa_ch_30y = np.mean(toa_ch[-(30*12):, :, :], axis=0)
...
ax1.set_title("Surface Albedo Adjustment " + nl.models_pl[mod] + " " + k_t[kl] + "\n" + "global mean " + 
...
print("\nGenerating nc file...\n")
out_name = f"Diff_to_CS{sfc_add}_sa" + exp_s + "_" + kl + out_ad[kl] + "_" + nl.models[mod] + ".nc"
f = Dataset(out_path + out_name, "w", format="NETCDF4")

# specify dimensions, create, and fill the variables; also pass the units
f.createDimension("time", np.shape(alb_ch)[0])
...
a_ad_nc = f.createVariable("sa_adj", "f4", ("time", "lat", "lon"))
...
for i in np.arange(0, int(n_yr*12), 12):
    ...
    a_ad_nc[i:i+12, :, :] = alb_ch[i:i+12, :, :] * rem_alke
...
f.description = (f"This file contains the {toa_sfc} radiative response difference " + 
                 f"between clear- and all-sky due to surface albedo change in the {exp_n} experiment for " +
                 nl.models_pl[mod] + ". This corresponds to the surface albedo adjustment for the cloud radiative\n" + 
