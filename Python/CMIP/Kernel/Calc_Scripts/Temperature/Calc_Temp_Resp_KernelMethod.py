"""
Calculate the temperature radiative response for CMIP members via the kernel method.
...
# output path
out_path = f"/Kernel_{toa_sfc}_RadResp/{exp_n}/{k_p[kl]}/T_Response/" + cs_pa + strat_path + out_path_ad[kl][k_st]
...
tim = ts_a4x_nc.variables["time"][:n_yr*12]
...
try:
    ...
    f_ts = sorted(glob.glob(data_path + f"/AddFiles_2d_piC/{ts_var}_Amon_{nl.models_n[mod]}_piControl" + 
                            ensemble_b_d + "*AddBe.nc"), key=str.casefold)
...
try:
    ...
    f_ts = sorted(glob.glob(data_path + f"/AddFiles_2d_piC/{ts_var}_Amon_{nl.models_n[mod]}_piControl" + 
                            ensemble_b_d + "*AddAf.nc"), key=str.casefold)
...
#%% calculate the surface temperature piControl 21-year running mean
...
pl_toa = take_rep * dts[:, None, :, :]


#%% calculate the vertically integrated Planck response
pl_resp = np.nansum(pl_toa, axis=1)
...
ta_toa_tot = da.nansum(ta_toa, axis=1)
...
ind = 6
l_num = 14
levels, norm, cmp = set_levs_norm_colmap(sig[kl]*t_toa_tot[-ind, :, :].compute(), l_num, c_min=-20, c_max=10, quant=0.99)
...
p1 = ax1.contourf(x, y, sig[kl]*t_toa_tot[-ind, :, :].compute(), levels=levels, norm=norm, cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
ax1.coastlines(resolution="50m", color="black", linewidth=0.5, zorder=101)
cb1.set_label("T response in Wm$^{-2}$K$^{-1}$")
ax1.set_title(cs_p + "T Reponse Last June " + kl + " Kernels\nGlobal Mean: " + 
              str(glob_mean(sig[kl]*t_toa_tot[-ind, :, :].compute(), lat, lon)) + " Wm$^{-2}$")
pl.show()
pl.close()

print(glob_mean(sig[kl]*t_toa_tot[-ind, :, :].compute(), lat, lon))
# raise Exception("")
"""
...
print("\nGenerating nc file...\n")
out_name = toa_sfc + "_RadResponse_ta_ts_lr" + exp_s + "_" + kl + out_ad[kl] + "_" + cs_n + nl.models[mod] + ".nc"
...
t_toa_nc = f.createVariable("t_resp", "f", ("time", "lat", "lon"))
ta_toa_nc = f.createVariable("ta_resp", "f4", ("time", "lat", "lon"))
ts_toa_nc = f.createVariable("ts_resp", "f4", ("time", "lat", "lon"))
lr_toa_nc = f.createVariable("lr_resp", "f4", ("time", "lat", "lon"))
# pl_toa_nc = f.createVariable("pl_resp", "f4", ("time", "lat", "lon"))
...
for i in np.arange(0, n_yr*12, n):
    # print("storing months " + str(i+1) + " to " + str(i + n_yr) + f" of {str(n_yr*12)}")
    t_toa_nc[i:(i+n), :, :] = t_toa_tot[i:(i+n), :, :].compute(scheduler='single-threaded')
    # ta_toa_nc[i:(i+n), :, :] = ta_toa_tot[i:(i+n), :, :].compute(scheduler='single-threaded')
    ts_toa_nc[i:(i+n), :, :] = ts_toa[i:(i+n), :, :].compute(scheduler='single-threaded')
    lr_toa_nc[i:(i+n), :, :] = laps_resp[i:(i+n), :, :].compute(scheduler='single-threaded')
    #pl_toa_nc[i:(i+n), :, :] = pl_resp_tot[i:(i+n), :, :].compute(scheduler='single-threaded')
    # print_one_line(str(np.round((i+n) / (n_yr*12) * 100, decimals=1)) + " %  ")
...
ts_toa_nc.description = sky_desc + f" {toa_sfc} radiative response to ts change, positive-" + k_pud[kl]
lr_toa_nc.description = sky_desc + f" {toa_sfc} radiative response due to lapse rate, positive-" + k_pud[kl]
#pl_toa_nc.description = sky_desc + " Planck TOA radiative response, positive-" + k_pud[kl]
...
# add attributes
f.description = (f"This file contains the {sky_desc} {toa_sfc} radiative response due to ta + ts change as well as the" +
                 f"\n lapse rate response in the {exp_n} experiment for " + nl.models_pl[mod] + 
                 ".\nNote that the lapse rate response can be calculated from this via lr_resp=ta_resp-pl_resp.\n"+
                 "The values correspond to monthly means.\nThe kernels are taken from " + k_t[kl] + "." + desc_add)
