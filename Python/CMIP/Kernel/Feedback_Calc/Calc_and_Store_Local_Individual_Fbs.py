"""
Calculate and store LOCAL individual feedbacks for arbitrary kernels. That is local TOA/SFC flux on GLOBAL mean tas/ts.
...
out_path = data_path + f"/Feedbacks_Local/Kernel/{a4x}/{k_p[kl]}/{t_var}_Based/{sfc_path}/" + cs_path
...
if not cs:
    ...
    res["Total"] = res["S"] + res["Q"] + res["LR"] + res["Pl"]
# end if else
...
for fb in res.keys():
    ...
    fbs[fb]["fb_e"] = np.zeros((2, len(lat), len(lon)))  # dimension 0 has length 2 for slope and p-value
    fbs[fb]["fb_l"] = np.zeros((2, len(lat), len(lon)))
    fbs[fb]["fb_t"] = np.zeros((2, len(lat), len(lon)))

    
    print("\nPerforming the linear regressions for " + fb + "...")
    for la in np.arange(len(lat)):
        for lo in np.arange(len(lon)):
            fbs[fb]["fb_e"][0, la, lo], yi, r, fbs[fb]["fb_e"][1, la, lo] = lr(tas[:20], res[fb][:20, la, lo])[:4]
            fbs[fb]["fb_l"][0, la, lo], yi, r, fbs[fb]["fb_l"][1, la, lo] = lr(tas[20:], res[fb][20:, la, lo])[:4]
            fbs[fb]["fb_t"][0, la, lo], yi, r, fbs[fb]["fb_t"][1, la, lo] = lr(tas[:], res[fb][:, la, lo])[:4]
        # end for lo
    # end for la
    
    # calculate the change in lapse rate feedback from early to late period
    fbs[fb]["dfb"] = fbs[fb]["fb_l"][0, :, :] - fbs[fb]["fb_e"][0, :, :]  # ignore the p-value for now
    
# end for fb


#%% regrid the local feedbacks to a given model grid
...
for fb in fbs.keys():
    ...
    fbs_rg[fb]["fb_e"] = np.zeros((2, len(lat_rg), len(lon_rg)))  # dimension 0 has length 2 for slope and p-value
    ...
    if ((mod != 21) & (cmip_v == 6)) | (cmip_v == 5):
        lat_o = lat + 90
        lat_t = lat_rg + 90    
        fbs_rg[fb]["fb_e"][:] = remap(lon_rg, lat_t, lon, lat_o, fbs[fb]["fb_e"][0, :, :], verbose=True)
        fbs_rg[fb]["fb_l"][:] = remap(lon_rg, lat_t, lon, lat_o, fbs[fb]["fb_l"][0, :, :])
        fbs_rg[fb]["fb_t"][:] = remap(lon_rg, lat_t, lon, lat_o, fbs[fb]["fb_t"][0, :, :])
    else:
        ...
        lon_o = lon - 180
        lon_t = lon_rg - 180        
        fbs_rg[fb]["fb_e"][:] = remap2(lon_t, lat_t, lon_o, lat_o, fbs[fb]["fb_e"][0, :, :], rad_of_i=1500000, 
                                       epsilon=0.5, fill_value=None)
        fbs_rg[fb]["fb_l"][:] = remap2(lon_t, lat_t, lon_o, lat_o, fbs[fb]["fb_l"][0, :, :], rad_of_i=1500000, 
                                       epsilon=0.5, fill_value=None)
        fbs_rg[fb]["fb_t"][:] = remap2(lon_t, lat_t, lon_o, lat_o, fbs[fb]["fb_t"][0, :, :], rad_of_i=1500000, 
                                       epsilon=0.5, fill_value=None)        
    # end if else
        
    # calculate the change in lapse rate feedback from early to late period
    fbs_rg[fb]["dfb"] = fbs_rg[fb]["fb_l"][0, :, :] - fbs_rg[fb]["fb_e"][0, :, :]  # ignore the p-value for now
...
f = Dataset(out_path + f"Local_{ascs_n}Feedbacks_{t_var}_Based_{kl}Kernel_{toa_sfc}_piC{run}Run_a{exp}_" +
...
for fb in res.keys():
    fb_e_nc = f.createVariable(fb + "_fb_e", "f4", ("2", "lat", "lon"))
    fb_l_nc = f.createVariable(fb + "_fb_l", "f4", ("2", "lat", "lon"))
    fb_t_nc = f.createVariable(fb + "_fb_t", "f4", ("2", "lat", "lon"))
    dfb_nc = f.createVariable(fb + "_dfb", "f4", ("lat", "lon"))
    ...
    dfb_rg_nc = f.createVariable(fb + "_dfb_rg", "f4", ("lat_rg", "lon_rg"))    
...
f.description = ("This file contains local annual mean radiative {toa_sfc} {ascs_desc} feedbacks (early, late, and " +
                 "total period as well as\n the change from early to late) for " + nl.models[mod] + 
                 f" based on global mean annual {t_var}. " +
                 "The first dimension contains the feedback values (slopes) in [0] as well as the p-values in [1].\n" + 
