"""
Calculate annual mean zonal mean vertical cross section for cloud.

Generates Fig. S17 in Eiselt and Graversen (2023), JCLI.
...
dsat_com = np.mean(dsat_com_nc.variables["dTREFHT"][:nyr, :, :, :], axis=1)
dsat_exp = np.mean(dsat_exp_nc.variables["dTREFHT"][:nyr, :, :, :], axis=1)
...
#%% set the length of the loaded files
com = com[:12*nyr, :, :, :]
exp = exp[:12*nyr, :, :, :]
...
syr = 20
eyr = 40
exp_20_40_zm = np.mean(exp_an_zm[syr:eyr, :, :], axis=0)
com_20_40_zm = np.mean(com_an_zm[syr:eyr, :, :], axis=0)
dexp_20_40_zm = np.mean(dexp_an_zm[syr:eyr, :, :], axis=0)
dcom_20_40_zm = np.mean(dcom_an_zm[syr:eyr, :, :], axis=0)
exp_20_40_zm_norm = np.mean(exp_zm_norm[syr:eyr, :, :], axis=0)
com_20_40_zm_norm = np.mean(com_zm_norm[syr:eyr, :, :], axis=0)

dcom_nt_20_40_norm = np.mean(dcom_nt_norm[syr:eyr, :], axis=0)
dcom_st_20_40_norm = np.mean(dcom_st_norm[syr:eyr, :], axis=0)
dexp_nt_20_40_norm = np.mean(dexp_nt_norm[syr:eyr, :], axis=0)
dexp_st_20_40_norm = np.mean(dexp_st_norm[syr:eyr, :], axis=0)

dcom_nt_20_40 = np.mean(dcom_nt[syr:eyr, :], axis=0)
dcom_st_20_40 = np.mean(dcom_st[syr:eyr, :], axis=0)
dexp_nt_20_40 = np.mean(dexp_nt[syr:eyr, :], axis=0)
dexp_st_20_40 = np.mean(dexp_st[syr:eyr, :], axis=0)
...
if var == "Q":
    ...
    dlevels = np.arange(-0.1, 0.11, 0.01)
    dticks = np.arange(-0.1, 0.11, 0.02)    
# end if elif
...
fig.suptitle(pl_title + f" fraction anomaly averaged over years {syr}-{eyr} CESM2-SOM")

# pl.savefig(pl_path + "SpecHum_dQ_nodQ_yr20to40_Avg.pdf", bbox_inches="tight", dpi=250)
# pl.savefig(pl_path + f"{var}_VertCrossSec_dQ_nodQ_yr{syr}to{eyr}_Avg.pdf", bbox_inches="tight", dpi=250)
