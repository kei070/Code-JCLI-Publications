"""
Calculate annual mean zonal mean vertical cross section for humidity (Q).

Generates Fig. S16 in Eiselt and Graversen (2023), JCLI.
...
#%% load the data
con = da.ma.masked_array(nc_con.variables[var], lock=True) * 1000
com = da.ma.masked_array(nc_com.variables[var], lock=True) * 1000
exp = da.ma.masked_array(nc_exp.variables[var], lock=True) * 1000
...
if len(f_exp) > 1:
    for i in np.arange(1, 40):
        ...
        com = da.concatenate([com, da.ma.masked_array(dataset_com.variables[var], lock=True) * 1000])
        exp = da.concatenate([exp, da.ma.masked_array(dataset_exp.variables[var], lock=True) * 1000])
        # dataset.close()
    # end for i
# end if
...
if len(f_con) > 1:
    for i in np.arange(1, len(f_con)):
        ...
        con = da.concatenate([con, da.ma.masked_array(dataset.variables[var], lock=True) * 1000])
        # dataset.close()
    # end for i
# end if
