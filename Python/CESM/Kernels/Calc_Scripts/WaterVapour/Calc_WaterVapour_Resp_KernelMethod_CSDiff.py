...
# output path
out_path = (data_path + f"/{case}/Rad_Kernel/Kernel_{toa_sfc}_RadResp/{k_p[kl]}/01_Diff_to_CS/" + strat_path + 
            out_path_ad[kl][k_st])
...
#%% interpolate the missing values of the q kernel
...
# regrid the field
print("\n\nRegridding kernels to CESM2-SOM grid using " + regrid_meth + "...\n")
...
#%% Q LW kernel
k_lw_plot = np.nanmean(np.nanmean(rem_qke_lw, axis=0), axis=-1)

l_num = 14
levels, norm, cmp = set_levs_norm_colmap(k_lw_plot, l_num, quant=1)

fig, ax1 = pl.subplots(nrows=1, ncols=1, figsize=(12, 6))
p1 = ax1.contourf(k_lw_plot, levels=levels, norm=norm, cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
cb1.set_label("Q LW kernel in Wm$^{-2}$K$^{-1}$100hPa$^{-1}$")
ax1.set_title("Q LW Kernel Annual Mean Zonal Mean")
pl.show()
pl.close()


#%% Q SW kernel
k_sw_plot = np.nanmean(np.nanmean(rem_qke_sw, axis=0), axis=-1)

l_num = 14
levels, norm, cmp = set_levs_norm_colmap(k_sw_plot, l_num, quant=0.975)

fig, ax1 = pl.subplots(nrows=1, ncols=1, figsize=(12, 6))
p1 = ax1.contourf(k_sw_plot, levels=levels, norm=norm, cmap=cmp, extend="both")
cb1 = fig.colorbar(p1, ax=ax1, ticks=levels)
cb1.set_label("Q SW kernel in Wm$^{-2}$K$^{-1}$100hPa$^{-1}$")
ax1.set_title("Q SW Kernel Annual Mean Zonal Mean")
pl.show()
pl.close()
...
q_lw_adj = rem_qke_lw[None, :, :, :, :] / dqdt * dq
q_sw_adj = rem_qke_sw[None, :, :, :, :] / dqdt * dq


#%% sum up the vertical levels
q_lw_adj_tot = da.nansum(q_lw_adj, axis=2)
q_sw_adj_tot = da.nansum(q_sw_adj, axis=2)


#%% a final test plot
p_ind = -10
pl_test = sig[kl] * q_lw_adj_tot[p_ind, 6, :, :].compute()
...
print("\nGenerating nc file...\n")
out_name = f"Diff_to_CS{sfc_add}_q_" + case + "_" + kl + out_ad[kl] + "_CESM2-SOM.nc"
...
q_lw_adj_nc = f.createVariable("q_lw_adj", "f4", ("year", "month", "lat", "lon"))
q_sw_adj_nc = f.createVariable("q_sw_adj", "f4", ("year", "month", "lat", "lon"))
...
q_lw_adj_nc[:] = q_lw_adj_tot.compute(scheduler='single-threaded')
q_sw_adj_nc[:] = q_sw_adj_tot.compute(scheduler='single-threaded')

tot_time = np.round((timeit.default_timer() - start) / 60, decimals=1)    
print("\nTime needed for storage: " + str(tot_time) + " min.")

q_lw_adj_nc.units = "W m^-2"
q_sw_adj_nc.units = "W m^-2"

lat_nc.units = "degrees_north"
lon_nc.units = "degrees_east"

q_lw_adj_nc.description = (f"difference between long wave clear and total sky radiative response change at {toa_sfc}" +
                            " due to " + dq_str + ", i.e. the lw " + dq_str + " adjustment; positive-" + k_pud[kl])
q_sw_adj_nc.description = (f"difference between short wave clear and total sky radiative response change at {toa_sfc}"+
...
f.description = (f"This file contains the {toa_sfc} long- and short-wave radiative response difference " + 
                 "between clear and total sky due to water vapour change in the abrupt4xCO2 experiment for " +
                 "CESM2-SOM. This corresponds to the water vapour adjustment for the cloud radiative\n" + 
...
pl.plot(glob_mean(np.mean(sig[kl]*q_lw_adj_tot, axis=1), lat, lon))