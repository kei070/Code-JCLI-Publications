...
# plot path
# pl_path = (direc + "/Uni/PhD/Tromsoe_UiT/Work/" + cmip + "/Plots/" + nl.models[mod] + "/Kernel/" + k_p[kl] + "/")
...
ta_adj_tot = da.nansum(ta_adj, axis=2)
...
#%% a final test plot
ind = 4
pl_test = sig[kl]*t_adj_tot[ind, 5, :, :].compute()
...
print("\nGenerating nc file...\n")
out_name = f"Diff_to_CS{sfc_add}_ta_ts_" + case + "_" + kl + out_ad[kl] + "_CESM2-SOM.nc"
...
t_adj_nc = f.createVariable("t_adj", "f4", ("year", "month", "lat", "lon"))
ts_adj_nc = f.createVariable("ts_adj", "f4", ("year", "month", "lat", "lon"))
ta_adj_nc = f.createVariable("ta_adj", "f4", ("year", "month", "lat", "lon"))
...
t_adj_nc[:] = t_adj_tot.compute(scheduler='single-threaded')
ts_adj_nc[:] = ts_adj
ta_adj_nc[:] = ta_adj_tot.compute(scheduler='single-threaded')
...
f.description = (f"This file contains the {toa_sfc} radiative response difference " + 
                 "between clear and total sky due to temperature change in the abrupt4xCO2 experiment for " +
                 "CESM2-SOM. This corresponds to the temperature adjustment for the cloud radiative\n" + 
...
pl.plot(glob_mean(np.mean(sig[kl]*t_adj_tot, axis=1), lat, lon))